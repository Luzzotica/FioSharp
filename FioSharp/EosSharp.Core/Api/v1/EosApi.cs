...
namespace FioSharp.Core.Api.v1
{
    ...
    { 
        ...
        public async Task<Dictionary<string, dynamic>> GetJson(string endpoint)
        {
            var url = string.Format("{0}/v1/chain/{1}", Config.HttpEndpoint, endpoint);
            Console.WriteLine(url);
            return await HttpHandler.GetJsonAsync<Dictionary<string, dynamic>>(url);
        }
        public async Task<Dictionary<string, dynamic>> PostJson(string endpoint, Dictionary<string, dynamic> data, bool reload = false)
        {
            var url = string.Format("{0}/v1/chain/{1}", Config.HttpEndpoint, endpoint);
            Console.WriteLine(url);
            return await HttpHandler.PostJsonAsync<Dictionary<string, dynamic>>(url, data);
        }
        public async Task<Dictionary<string, dynamic>> PostJsonWithCache(string endpoint, Dictionary<string, dynamic> data, bool reload = false)
        {
            var url = string.Format("{0}/v1/chain/{1}", Config.HttpEndpoint, endpoint);
            Console.WriteLine(url);
            return await HttpHandler.PostJsonWithCacheAsync<Dictionary<string, dynamic>>(url, data, reload);
        }

        
        public async Task<GetInfoResponse> GetFioBalance()
        {
            var url = string.Format("{0}/v1/chain/get_fio_balance", Config.HttpEndpoint);
        ...
        public async Task<GetActorResponse> GetActor(GetActorRequest data)
        {
            var url = string.Format("{0}/v1/history/get_controlled_accounts", Config.HttpEndpoint);
            return await HttpHandler.PostJsonAsync<GetActorResponse>(url, data);
        }
    }
}
